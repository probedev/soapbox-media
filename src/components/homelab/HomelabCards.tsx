    ...
    <div className="bg-card border border-border rounded-lg p-5 shadow-sm flex flex-col">
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="font-semibold text-foreground">
          <span className="text-ink-faintest font-mono text-sm mr-2">#{n}</span>{title}
        </h3>
      </div>
      <p className="text-xs text-muted-foreground mt-1 mb-4">{hook}</p>
...
export function PulseCard({ d }: { d: Pulse }) {
  const stat = (label: string, value: string, delta?: number) => (
    <div className="border border-muted rounded-md p-3">
      ...
      {delta !== undefined && (
        <div className={`text-xs mt-1 ${delta >= 0 ? "text-emerald-600" : "text-ink-faint"}`}>
      ...
      {d.map((r) => {
        ...
        return (
          <div key={r.issue} className="flex items-center gap-2 text-xs">
            <div className="w-36 truncate text-ink-body text-right shrink-0">{r.issue}</div>
            <div className="flex-1 h-4 flex rounded overflow-hidden bg-muted">
              <div style={{ width: `${lp}%`, background: BLUE, opacity: 0.85 }} />
              <div style={{ width: `${100 - lp}%`, background: RED, opacity: 0.85 }} />
            </div>
            <div className="w-12 tabular-nums text-ink-faint shrink-0">{Math.round(lp)}/{Math.round(100 - lp)}</div>
          </div>
        );
      })}
      <div className="text-[10px] text-ink-faint pt-1">weighted share of left- vs right-aligned mentions · 7d · sorted by volume</div>
    </div>
  );
}
    ...
    <div className="overflow-x-auto">
      <table className="text-xs w-full">
        <thead>
          <tr>
            <th />
            {weekLabels.map((l) => <th key={l} className="font-normal text-ink-faint pb-1 px-0.5">{l}</th>)}
          </tr>
        </thead>
        <tbody>
          {issues.map((row) => (
            <tr key={row.issue}>
              <td className="pr-2 text-ink-body whitespace-nowrap max-w-[150px] truncate">{row.issue}</td>
              {row.weeks.map((c, i) => {
      ...
      <div className="text-[10px] text-ink-faint pt-2">cell saturation = volume · hue = weekly lean · 8 weeks</div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2.5">
        ...
        <div key={r.issue} className="text-xs">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 relative h-5">
              ...
              <div className="absolute inset-y-2 bg-input/60 rounded" style={{
      ...
      <div className="text-[10px] text-ink-faint pt-1">
        <span className="inline-block w-2 h-2 rounded-full bg-muted-foreground mr-1" />legacy ·{" "}
...
export function RisersCard({ d }: { d: RiserRow[] }) {
  return (
    <table className="w-full text-xs">
      ...
      <tbody>
        {d.map((r) => (
          <tr key={r.channel} className="border-t border-muted">
            <td className="py-1.5">
              <span className="font-medium text-ink-strong">{r.channel}</span>
              <span className={`ml-1.5 text-[9px] px-1 rounded ${r.lean === "R" ? "bg-red-50 text-red-700" : r.lean === "L" ? "bg-blue-50 text-blue-700" : "bg-muted text-ink-muted"}`}>{r.lean}</span>
              <span className="ml-1 text-[9px] text-ink-faint">{r.cohort === "legacy" ? "legacy" : ""} · {fmtReach(r.reach)}</span>
            </td>
            <td className="text-right tabular-nums text-ink-muted">{r.shareNow}%</td>
            <td className={`text-right tabular-nums font-medium ${r.shareDelta > 0 ? "text-emerald-600" : "text-ink-faint"}`}>
...
export function FusesCard({ d }: { d: FuseRow[] }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      {d.map((f) => (
        <div key={f.issue} className="border border-muted rounded-md p-3">
          <div className="flex items-baseline justify-between">
            <div className="text-xs font-medium text-ink-strong truncate pr-2">{f.issue}</div>
            <div className={`text-sm font-semibold tabular-nums ${f.ratio >= 1.5 ? "text-amber-600" : "text-muted-foreground"}`}>{f.ratio}×</div>
          </div>
...
export function StripsCard({ d }: { d: Strip[] }) {
  return (
    <div className="grid grid-cols-3 gap-3">
      {d.map((s) => (
        <div key={s.issue} className="border border-muted rounded-md p-2">
          <div className="text-[11px] font-medium text-ink-strong truncate">{s.issue}</div>
    ...
    <div className="space-y-3 max-h-[340px] overflow-y-auto pr-1">
      {d.map((r, i) => (
        <div key={i} className="border-l-2 pl-3" style={{ borderColor: leanColor(r.sent) }}>
          <p className="text-xs text-ink-strong leading-relaxed">&ldquo;{r.quote.length > 220 ? r.quote.slice(0, 220) + "…" : r.quote}&rdquo;</p>
          <div className="text-[10px] text-ink-faint mt-1">
            <span className="font-medium text-ink-muted">{r.channel}</span> · {r.issue} ·{" "}
    ...
    <div className="space-y-2">
      {d.map((r) => (
        <div key={r.who} className="text-xs">
          <div className="flex items-center gap-2">
            <div className="w-32 truncate font-medium text-ink-strong text-right shrink-0">{r.who}</div>
            <div className="flex-1 h-3.5 bg-muted rounded overflow-hidden">
              <div className="h-full bg-ink-body rounded" style={{ width: `${(r.count / max) * 100}%` }} />
            </div>
            <div className="w-8 tabular-nums text-muted-foreground shrink-0">{r.count}</div>
          </div>
          <div className="ml-34 pl-[8.5rem] text-[10px] text-ink-faint truncate">
            by {r.by.map((b) => `${b.channel} (${b.n})`).join(" · ")}
          </div>
        </div>
      ))}
      <div className="text-[10px] text-ink-faint pt-1">times named inside scored quotes by OTHER shows · 30d · string-match v1</div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="text-xs text-ink-muted mt-2">
        <span className="font-semibold text-foreground">{pctExtreme}%</span> of this week&apos;s {mentions.toLocaleString()} mentions
